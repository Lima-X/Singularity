...
#pragma region VisualSingularity task objects
	...
	public:
		...
		IVisualWorkObject::enum_t GetWorkObjectState() const override {
	...
	public:
		...
		SingularityException* GetExceptionReport() const {
	...
	private:
		void DoLoadFileAsyncProcessing() {
			...
			catch (const SingularityException& ExceptionInforamtion) {

				// Do internal cleanup and complete this request
				ExitExceptionCopy = std::make_unique<SingularityException>(ExceptionInforamtion);
				SymbolServer.release();
				TargetImage.release();
				SPDLOG_LOGGER_CALL(spdlog::default_logger(),
					ExceptionInforamtion.StatusCode == SingularityException::STATUS_ABORTED_OBJECT_IN_CALL ?
					spdlog::level::warn : spdlog::level::err,
					"Load task failed with [{}]: \"{}\"",
					ExceptionInforamtion.what(),
					ExceptionInforamtion.ExceptionText);
				ImageLoadState = ExceptionInforamtion.StatusCode == SingularityException::STATUS_ABORTED_OBJECT_IN_CALL ?
					VisualLoadImageObject::STATE_ABORTED : VisualLoadImageObject::STATE_ERROR;
			}
		}

		std::unique_ptr<SingularityException> ExitExceptionCopy; // under some states this maybe valid and contains exact information
